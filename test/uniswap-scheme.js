...
const UniswapTarget = artifacts.require('UniswapTarget');
...
const deploy = async (accounts) => {
  ...
  setup.target = await UniswapTarget.new();
  return setup;
};

contract.only('UniswapScheme', (accounts) => {
  let setup;
  ...
  context('# initialize', () => {
    ...
    it('it accepts proposal', async () => {
      ...
      const tx2 = await setup.votingMachine.absoluteVote.vote(proposalId, 1, 0, helpers.NULL_ADDRESS, { from: root });
    });
  });
});
